...
# attn_drop=0. ljq attn_drop=0.2
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0.2, proj_drop=0.15, window_size=7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 只保留局部窗口内的注意力分数
        attn_mask = torch.ones((N, N), device=x.device)  # 注意力掩码
        for i in range(N):
            start = max(0, i - self.window_size // 2)
            end = min(N, i + self.window_size // 2 + 1)
            attn_mask[i, start:end] = 1
        attn_mask = attn_mask.unsqueeze(0).expand(B, -1, -1)  # 扩展到批量维度
        attn_mask = attn_mask.to(x.device)
        #attn = attn.masked_fill(attn_mask == 0, float('-inf'))  # 应用掩码
...
class ConvTransBN(nn.Module):  # (convolution => [Norm] => GELU)
    def __init__(self, in_channels, out_channels):
        ...
        # 归一化层，使用LayerNorm，normalized_shape调整为out_channels
        self.norm = nn.LayerNorm(out_channels)
        
        # 激活函数使用GELU
        self.activation = nn.GELU()

        # 如果输入输出通道数相同，则使用skip connection
    ...
    def forward(self, x):
        ...
        # 经过归一化
        out = self.norm(out.transpose(1, 2))  # 转换为[batch_size, seq_len, channels] 
        out = out.transpose(1, 2)  # 恢复到原始形状
        
        # 经过激活函数
        out = self.activation(out)
        
        # 如果启用skip connection，直接加上输入
        if self.use_skip:
            out += x
